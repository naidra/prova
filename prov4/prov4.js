(function() {
		...
		if (window.innerWidth >= 650) {
			...
			for (var i = 0; i < sec.length; i++) {
				if (stt >= sec[i].offsetTop && stt < sec[i].offsetTop + sec[i].offsetHeight) {
					var aka = sec[i].getAttribute("data-w");
					an[aka].style.border = "2px solid #fff";
				} else if (stt >= sec[i].offsetTop + sec[i].offsetHeight || stt < sec[i].offsetTop) {
					var aka = sec[i].getAttribute("data-w");
					an[aka].style.border = "2px solid transparent";
				}
			}
			...
			for (var i = 0; i < sec.length; i++) {
				if (stt >= sec[i].offsetTop && stt < sec[i].offsetTop + sec[i].offsetHeight) {
					var aka = sec[i].getAttribute("data-w");
					an[aka].style.color = "green";
					an[aka].style.borderLeft = "15px solid green";
				} else if (stt >= sec[i].offsetTop + sec[i].offsetHeight || stt < sec[i].offsetTop) {
					var aka = sec[i].getAttribute("data-w");
					an[aka].style.color = "#fff";
					an[aka].style.borderLeft = "0px solid green";
				}
			}
	...
	function slip() {
		var dee = document.querySelector("div.days"),
			...
			dee.innerHTML = "<p>Ju dëshirojmë një të <span>" + dNames[dd] + "</span> të mirë.</p>";
		setTimeout(function() {
			dee.style.opacity = "0";
			dee.style.left = "100%";
	...
	function all() {
		...
		document.querySelector("nav > h1").addEventListener("click", menu, false);
